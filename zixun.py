...
API_URL = os.environ.get('MLION_API_URL', 'https://api.mlion.ai/v2/api/news/real/time?language=cn&time_zone=Asia%2FShanghai&num=100&page=1&client=mlion&is_hot=Y')
...
def get_latest_news():
    ...
    if not MLION_API_KEY:
        print("❌ 错误：你还没有配置 MLION_API_KEY！请在 Secrets 中配置或直接修改代码。")
        return None
    ...
    try:
        ...
        # 双重检查 API 内部错误码
        if isinstance(data, dict) and data.get("code") != 0 and data.get("code", 0) != 200:
            print(f"API 错误: code={data.get('code')}, message={data.get('message', data.get('msg', 'Unknown'))}")
            return None
        ...
        # 简单去重
        current_fingerprint = latest_news.get('id') or latest_news.get('pub_time') or latest_news.get('title')
...
def format_message(news):
    ...
    if isinstance(time_str, (int, float)):
        ...
        import datetime
        try:
            time_str = datetime.datetime.fromtimestamp(int(time_str)).strftime('%Y-%m-%d %H:%M:%S')
        except:
            pass
